...
Статус: v1.6 (в коде внедрен go-telegram/bot и start_payload flow)
Дата обновления: 2026-03-10
Основание: `tz.md`, `telegram-bot-flow.md`

## 1) Цель
Собрать и реализовать Telegram-бот + backend + БД + интеграцию с ЮKassa + админ-панель для управления платным доступом в закрытые Telegram-чаты по подписке.
...
- Валюта: только RUB.
- Интеграция с онлайн-кассой (54-ФЗ): не включать в MVP, только ЮKassa.
...
- Автопродление: использовать нативный recurring-механизм ЮKassa (автоплатеж).
...
- Создание платежа через ЮKassa и обработка webhook.
...
- Интеграции: Telegram Bot API + ЮKassa Payments API + webhook.
- Деплой: VPS, reverse proxy (Nginx/Caddy), TLS, systemd/docker compose.

## 5) Доменная модель (уточненный черновик)
- `connectors`
  - `id` (внутренний UUID), `start_payload` (unique, например `in-94db7d6813507bc`)
...
- `payments`
  - `id`, `user_id`, `subscription_id`, `connector_id`, `provider` (`yookassa`), `provider_payment_id`, `amount`, `currency`, `status`, `paid_at`, `raw_payload`, timestamps
- `user_consents`
...
3. Оплата
- Кнопка «Оплатить» создаёт платеж в ЮKassa с idempotence key.
...
- Статус: `частично`.
- Сделано: HTTP-сервер, базовые handlers, in-memory store.
- Осталось: PostgreSQL и миграции.
...
- Осталось для полного паритета: привязка кнопки `Оплатить` к реальному checkout URL ЮKassa.

### Итерация 3: Платежи ЮKassa
...
- Автопролонгация через нативный recurring-автоплатеж ЮKassa.
...
4. `Закрыт` Сценарий recurring в ЮKassa? -> Используем нативный механизм ЮKassa через автоплатеж.
...
7. `Закрыт` Нужна отдельная онлайн-касса? -> Нет, только ЮKassa.
