...
- [ ] Упростить экран коннекторов под механическую операторскую работу: меньше визуального шума, меньше лишних блоков, меньше "нейросеточного" интерфейсного оверхеда.
- [ ] Переписать labels и hints в форме коннектора в более очевидный операторский язык, чтобы каждое поле сразу объясняло действие и не требовало догадки.
- [ ] Довести operational view `коннектор -> подписка -> пользователь`: быстрее показывать, кто реально подключен, где активен текущий период, где только следующий период и где включен автоплатеж.
- [ ] Довести фильтрацию и сортировку коннекторов до production-операторского уровня: статусы, access mode, активные пользователи, автоплатеж, недавняя активность.
- [ ] Финально вычистить ложные "двойные подписки" и related status confusion по всему продукту: бот, recurring pages, админка, billing, users, operational tables.
- [ ] Принять окончательное UX-решение по period model в форме коннектора: что остаётся в основном интерфейсе для обычных ежемесячных тарифов, а что уходит в advanced/test-only path.
